...
function ClientList({
  ...
  const fetchClients = async () => {
    try {
      ...
      // 3. Récupérer les clients pour ces propriétaires (compatibilité ancienne/nouvelle colonne)
      let clientsData: any[] | null = null;
      let clientsError: any = null;

      // Tentative avec la nouvelle FK 'fk_reminder_profiles' (colonne clients.reminder_profiles)
      try {
        const res1 = await supabase
          .from("clients")
          .select("*, reminderProfile:reminder_profile!fk_reminder_profiles(*)")
          .in("owner_id", allOwnerIds)
          .order("company_name");
        clientsData = (res1 as any).data as any[] | null;
        clientsError = (res1 as any).error;
      } catch (e) {
        clientsError = e;
      }

      // Fallback: ancienne relation implicite 'reminder_profile'
      if (clientsError || !clientsData) {
        try {
          const res2 = await supabase
            ...
            .in("owner_id", allOwnerIds)
            .order("company_name");
          clientsData = (res2 as any).data as any[] | null;
          clientsError = (res2 as any).error;
        } catch (e2) {
          clientsError = e2;
        }
      }
